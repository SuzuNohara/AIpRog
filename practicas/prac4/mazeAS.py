...
def solve_maze(maze, mouse):
    ...
    heap = []
    heapq.heappush(heap, (heuristic(start, end), 0, start, []))
    visited = set()
    visited.add(start)
    mouse.gnodes = 1

    while heap:
        priority, cost, (x, y), path = heapq.heappop(heap)
        mouse.enodes += 1
        ...
        for action, (dx, dy) in zip(['l', 'r', 'u', 'd'], [(0, -1), (0, 1), (-1, 0), (1, 0)]):
            nx, ny = x + dx, y + dy
            next_pos = (nx, ny)
            if 0 <= nx < len(maze) and 0 <= ny < len(maze[0]) and maze[nx][ny] == 1 and next_pos not in visited:
                visited.add(next_pos)
                new_cost = cost + 1
                priority = new_cost + heuristic(next_pos, end)
                heapq.heappush(heap, (priority, new_cost, next_pos, path + [action]))
                mouse.gnodes += 1
    return False
